...
export CONSUL_HTTP_ADDR="http://${CONSUL_IP}:${CONSUL_PORT}"
...
VAULT_INIT_STATUS=$(vm_exec "$VM_VAULT" \
  "VAULT_ADDR=http://127.0.0.1:${VAULT_PORT} vault status 2>/dev/null | awk '/^Initialized/{print \$2}'" || echo "false")
...
if [[ "$VAULT_INIT_STATUS" == "true" ]]; then
  ...
  VAULT_INIT_OUTPUT=$(vm_exec "$VM_VAULT" \
    "VAULT_ADDR=http://127.0.0.1:${VAULT_PORT} vault operator init \
...
vm_exec "$VM_VAULT" \
  "VAULT_ADDR=http://127.0.0.1:${VAULT_PORT} vault operator unseal '${VAULT_UNSEAL_KEY}'"
ok "Vault unsealed"
...
if [[ -f "${SECRETS_DIR}/consul_bootstrap_token" ]]; then
  ...
  CONSUL_BOOTSTRAP_OUTPUT=$(vm_exec "$VM_CONSUL" \
    "CONSUL_HTTP_ADDR=http://127.0.0.1:${CONSUL_PORT} consul acl bootstrap -format=json")
...
NOMAD_SERVER_STATUS=$(vm_exec "$VM_NOMAD_SERVER" \
  "NOMAD_ADDR=http://127.0.0.1:${NOMAD_PORT} nomad server members 2>/dev/null" || echo "error")
...
NODE_STATUS=$(vm_exec "$VM_NOMAD_SERVER" \
  "NOMAD_ADDR=http://127.0.0.1:${NOMAD_PORT} nomad node status 2>/dev/null" || echo "")
